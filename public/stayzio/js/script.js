...
(function () {
    ...
    // ========== Location Dropdown ==========
    // Removed: this legacy handler was stealing clicks via stopPropagation(),
    // preventing the new inline autosuggest handler on #locationDropdown from ever firing.
    // Autosuggest logic now lives entirely in the home page's own inline <script> block.

    // ========== FULL YEAR DATE PICKER - SINGLE DATE SELECTION ==========
    ...
    // Consolidated global-to-block tracking down to a single date object
    ...
    // Refactored to only evaluate a single date reference
    function updateDateDisplay() {
        if (selectedDate) {
            dateRangeDisplay.innerHTML = `${formatDate(selectedDate)} <i class="fas fa-chevron-down"></i>`;
            if (selectedRangeSpan) {
                selectedRangeSpan.innerText = `${formatDateFull(selectedDate)}`;
            }
        }
    }
        ...
        monthsToShow.forEach((monthDate) => {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                dayDiv.addEventListener('click', (e) => {
                    ...
                    // Immediately assign single picked date and update state
                    selectedDate = dayDate;
                    updateDateDisplay();
                    
                    // Close the popup window right away upon selection
    ...
    // Stripped range-based scanning and simplified CSS class token assignment
    ...
    function updateFooterText() {
        ...
        if (selectedDate) {
            ...
            selectedRangeSpan.innerText = 'No date selected';
        }
    }
    ...
    // CHANGED LOGIC: Pull dynamic current date instead of fixed static 20 Apr 2026
    const defaultDate = new Date();
    selectedDate = defaultDate;
    
    if (dateRangeDisplay) {
        dateRangeDisplay.innerHTML = `${formatDate(defaultDate)} <i class="fas fa-chevron-down"></i>`;
    }
    if (selectedRangeSpan) {
        selectedRangeSpan.innerText = `${formatDateFull(defaultDate)}`;
    }
