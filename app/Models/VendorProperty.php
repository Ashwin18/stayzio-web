...
class VendorProperty extends Model
{
    ...
    public function getAmenitiesListAttribute()
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            if (is_string($amenities)) {
                ...
                if (is_array($decoded)) { $amenities = $decoded; break; }
                if (is_string($decoded)) { $amenities = $decoded; continue; }
                break;
            } else { break; }
        }
    ...
    /**
     * Create hotel + room records when admin approves
     */
    public function createHotelAndRoom(array $adminData = [])
    {
        $langId    = DB::table('languages')->where('is_default', 1)->value('id');
        $catId     = DB::table('hotel_categories')->value('id');
        $countryId = DB::table('countries')->where('name', 'like', '%India%')->value('id');
        ...
        $hotelId = DB::table('hotels')->insertGetId([
            ...
            'couple_friendly' => ($this->allow_same_city_couples === 'yes' || $this->allow_outstation_couples === 'yes') ? 1 : 0,
            'created_at'      => now(),
            'updated_at'      => now(),
        ]);
        ...
        // Get perk/restriction titles for description
        $perkIds = json_decode($this->selected_perks ?? '[]', true) ?: [];
        $restrictionIds = json_decode($this->selected_restrictions ?? '[]', true) ?: [];
        $perkTitles = $perkIds; // Store IDs directly, blade does the lookup
        $restrictionTitles = $restrictionIds; // Store IDs directly, blade does the lookup

        // Build perks/restrictions JSON for hotel_contents
        $perksJson = json_encode($perkIds);
        // Build restrictions in format blade expects: [{id:X, type:'not_allowed'}, ...]
        $restrictionsArr = [];
        if (!empty($restrictionIds)) {
            $restrData = DB::table('hotel_restrictions')->whereIn('id', $restrictionIds)->get();
            foreach($restrData as $rd) {
                $restrictionsArr[] = ['id' => (int)$rd->id, 'type' => $rd->default_type ?? 'not_allowed'];
            }
        }
        $restrictionsJson = json_encode($restrictionsArr);
        ...
        DB::table('room_contents')->insert([
            ...
            'room_category' => DB::table('room_categories')->where('status', 1)->where('name', $this->room_type)->value('id')
                ?? DB::table('room_categories')->where('status', 1)->value('id'),
            'description' => $this->room_type . ' with ' . implode(', ', $perkTitles ?: $this->amenities_list),
            'created_at'  => now(),
            'updated_at'  => now(),
        ]);

        // Create hourly room prices
        ...
        foreach ([
            ...
            $bhId = $bhIds[strval($p['hour'])] ?? DB::table('booking_hours')->value('id');
        ...
        // Link property to hotel
        // Insert gallery images and copy to correct folders
        ...
        foreach([$galleryDir, $logoDir, $featureDir] as $d) { if(!is_dir($d)) mkdir($d, 0755, true); }

        if (!empty($imgData['main'])) {
            DB::table('hotel_images')->insert(['hotel_id'=>$hotelId, 'image'=>$imgData['main'], 'created_at'=>now(), 'updated_at'=>now()]);
            // Copy to gallery, logo, and featureImage folders
            $src = $hotelImgDir . $imgData['main'];
            if(file_exists($src)) {
                @copy($src, $galleryDir . $imgData['main']);
                @copy($src, $logoDir . $imgData['main']);
                @copy($src, $featureDir . $imgData['main']);
            }
        }
        if (!empty($imgData['gallery'])) {
            foreach($imgData['gallery'] as $gImg) {
                DB::table('hotel_images')->insert(['hotel_id'=>$hotelId, 'image'=>$gImg, 'created_at'=>now(), 'updated_at'=>now()]);
                $src = $hotelImgDir . $gImg;
                if(file_exists($src)) @copy($src, $galleryDir . $gImg);
            }
        }
