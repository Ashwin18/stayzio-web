...
class CheckPackageLimits
{
    ...
    public function handle(Request $request, Closure $next, $feature, $method)
    {
        if (Auth::check()) {
            ...
            if (empty($package)) {
                // No package found - check if vendor has any lifetime membership (package_id=9)
                $lifetimeMembership = \App\Models\Membership::where('vendor_id', $vendor->id)
                    ->where('status', 1)
                    ->where('expire_date', '9999-12-31')
                    ->first();
                if ($lifetimeMembership) {
                    // Treat as unlimited - bypass package limits
                    return $next($request);
                }
                Session::flash('warning', __('Please contact admin to activate your account') . '!');
