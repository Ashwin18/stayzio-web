...
class VendorPermissionHelper
{
  ...
  public static function packagePermission(int $vendor_id)
  {
    ...
    // No subscription model - always use free unlimited Package 9
    $package = Package::query()->find(9);
    return $package ? $package : new Package([
  ...
  public static function currentPackagePermission(int $userId)
  {
    ...
    if (isset($currentPackage)) {
        return Package::query()->findOrFail($currentPackage->package_id);
    }
    // No active membership - fall back to free Package 9
    return Package::query()->find(9);
  }
