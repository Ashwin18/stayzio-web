...
class VendorManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Attach pending property submission counts (not yet approved into live hotels)
        $vendorIds = $vendors->pluck('id');
        $pendingCounts = \Illuminate\Support\Facades\DB::table('vendor_properties')
            ->whereIn('vendor_id', $vendorIds)
            ->whereIn('status', ['pending', 'draft'])
            ->selectRaw('vendor_id, COUNT(*) as cnt')
            ->groupBy('vendor_id')
            ->pluck('cnt', 'vendor_id');
        $vendors->getCollection()->transform(function ($v) use ($pendingCounts) {
            $v->pending_properties_count = $pendingCounts[$v->id] ?? 0;
            return $v;
        });
    ...
    public function updateAccountStatus(Request $request, $id)
    {
        ...
        if ($request->account_status == 1) {
            ...
            // Auto-assign free unlimited Package 9 membership if vendor has none
            $hasActiveMembership = \App\Models\Membership::where('vendor_id', $user->id)
                ->where('status', 1)
