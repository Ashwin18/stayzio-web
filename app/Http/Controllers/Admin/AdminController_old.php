...
class AdminController extends Controller
{
    ...
    public function redirectToDashboard()
    {
        ...
        // ── Keep original variables the blade may already reference ──
        ...
        $information['locations']        = Room::get()->count();
        ...
        $information['todayRevenue']  = \App\Models\Booking::whereDate('created_at', $today)
            ->where('payment_status', 1)->sum('grand_total');
        $information['pendingApprovals'] = DB::table('hotels')->where('approval_status', 0)->count();

        // ── Today's activity ──
        $information['todayCheckins']  = \App\Models\Booking::whereDate('check_in_date', $today)
            ->where('payment_status', 1)->count();
        $information['todayCheckouts'] = \App\Models\Booking::whereDate('check_out_date', $today)
            ->where('payment_status', 1)->count();
        $information['pendingVendors'] = \App\Models\Vendor::where('status', 0)
            ->where('id', '!=', 0)->count();

        // ── KPI: Bookings ──
        $information['totalBookings']  = \App\Models\Booking::count();
        $information['activeBookings'] = \App\Models\Booking::where('payment_status', 'paid')->count();

        // ── KPI: Revenue ──
        $information['totalRevenue'] = \App\Models\Booking::where('payment_status', 'paid')->sum('grand_total');
        $information['monthRevenue'] = \App\Models\Booking::where('payment_status', 'paid')
            ->whereDate('created_at', '>=', $monthStart)->sum('grand_total');
        ...
        $information['recentBookings'] = \App\Models\Booking::with(['hotel.hotel_contents', 'hotelRoom.room_content'])
            ->latest()->limit(8)->get();
