...
class HotelController extends Controller
{
    ...
    public function create($id)
    {
        ...
        $information['categories'] = \App\Models\HotelCategory::where('language_id',
            optional($languages->where('is_default',1)->first())->id)->get();
        $information['countries']  = \App\Models\Location\Country::orderBy('name')->get();
        $information['numberoffImages'] = 99999999;

        if ($id != 0) {
            $package = VendorPermissionHelper::packagePermission($id);
            if ($package != '[]') {
                $information['numberoffImages'] = $package->number_of_images_per_hotel ?? 99999999;
                return view('admin.hotel-management.create', $information);
            } else {
                Session::flash('warning', __('Please assign a package to this vendor first') . '!');
    ...
    public function store(HotelStoreRequest $request)
    {
        ...
        if ($totalHotelAdded < $current_packageHotel) {
            ...
            // Badge flags (checkboxes - 0 if not submitted)
            $in['couple_friendly']   = $request->has('couple_friendly') ? 1 : 0;
            $in['local_id_accepted'] = $request->has('local_id_accepted') ? 1 : 0;
            $in['foreign_guests']    = $request->has('foreign_guests') ? 1 : 0;
            $in['age_restriction']   = $request->has('age_restriction') ? 1 : 0;
            ...
            Session::flash('success', __('New Hotel added successfully') . '!');

            return Response::json(['status' => 'success'], 200);
        } else {
            Session::flash('warning', __('Hotel limit reached or exceeded') . '!');
    ...
    public function bulkSort(Request $request)
{
    ...
    if (!empty($ids)) {
        foreach ($ids as $index => $id) {
            \App\Models\Hotel::where('id', $id)->update([
        ...
        return redirect()->back()->with('success', 'Hotel sort order updated successfully!');
    }

    if ($request->wantsJson() || $request->ajax()) {
        return response()->json(['success' => false], 422);
    }
    return redirect()->back()->with('error', 'No updates found.');
}

    public function popularityOrder()
    {
        $defaultLang = Language::where('is_default', 1)->first() ?? Language::first();
        $language_id = $defaultLang->id ?? 1;

        $hotels = \App\Models\Hotel::with(['hotel_contents' => function ($q) use ($language_id) {
                $q->where('language_id', $language_id);
            }, 'vendor'])
            ->orderByRaw('CASE WHEN serial_number = 0 THEN 1 ELSE 0 END ASC, serial_number ASC, id ASC')
            ->get();

        return view('admin.hotel-management.popularity-order', compact('hotels', 'defaultLang'));
    }
    ...
    public function update(HotelUpdateRequest $request, $id)
    {
        ...
        foreach ($languages as $language) {
            ...
            if (
                ...
                $hotelContent->amenities = json_encode($amenities);
                $hotelContent->description = Purifier::clean($request[$code . '_description'], 'youtube');
                // Save perks (array of IDs from master data)
                $hotelContent->perks = json_encode(array_values(array_filter($request->input($code . '_perks', []))));
                // Save policies (array of IDs from master data)
                $hotelContent->policies = json_encode(array_values(array_filter($request->input($code . '_policies', []))));
                // Save restrictions (not language-specific - save on default language only)
                if ($language->is_default) {
                    $restrictionsInput = $request->input('restrictions', []);
                    $selectedRestrictions = [];
                    foreach ($restrictionsInput as $rid => $rdata) {
                        if (!empty($rdata['selected'])) {
                            $selectedRestrictions[] = ['id' => (int)$rid, 'type' => $rdata['type'] ?? 'not_allowed'];
                        }
                    }
                    $hotelContent->restrictions = json_encode($selectedRestrictions);
                }
                $hotelContent->meta_keyword = $request[$code . '_meta_keyword'];
                $hotelContent->meta_description = $request[$code . '_meta_description'];
                $hotelContent->save();
        ...
        // Save badge flags on update
        $hotel = \App\Models\Hotel::find($id);
        if ($hotel) {
            $hotel->couple_friendly   = $request->has('couple_friendly') ? 1 : 0;
            $hotel->local_id_accepted = $request->has('local_id_accepted') ? 1 : 0;
            $hotel->foreign_guests    = $request->has('foreign_guests') ? 1 : 0;
            $hotel->age_restriction   = $request->has('age_restriction') ? 1 : 0;
            $hotel->save();
        }
    ...
    public function approveHotel(Request $request, $id)
    {
        ...
        $hotel->update(['approval_status' => 1, 'status' => 1]);
        
        // Notify vendor
        $vendor = $hotel->vendor;
        if ($vendor) {
            \Illuminate\Support\Facades\Session::flash('success', 'Hotel approved and is now live on the platform.');
        }
    ...
    {
        $hotel = \App\Models\Hotel::findOrFail($id);
        $hotel->update(['approval_status' => 2, 'status' => 0]);
        return redirect()->back()->with('success', 'Hotel rejected.');
    }

    public function pendingHotels(Request $request)
    {
        $language = \App\Models\Language::where('code', $request->language ?? 'en')->firstOrFail();
        $information['language'] = $language;
        $information['langs'] = \App\Models\Language::all();
        $information['pendingHotels'] = \App\Models\Hotel::with(['hotel_contents', 'vendor'])
            ->where('approval_status', 0)
            ->latest()
            ->paginate(15);
        return view('admin.hotel-management.pending-approvals', $information);
    }
