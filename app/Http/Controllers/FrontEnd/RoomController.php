   ...
   public function index(Request $request)
{
    ...
    if ($checkInDates) {
        ...
        foreach ($rooms as $room) {
            ...
            if ($dailyInventory) {
                // Blanket 'Blocked' (holiday/maintenance) always excludes the room
                if ($dailyInventory->status === 'Blocked') {
                    $roomtimeIds[] = $id;
                    continue;
                }
                // Per-duration sold-out check, consistent with listing/detail page logic
                $reqHour = $hour ?? 24;
                $durationField = $reqHour <= 3 ? '3hrs' : ($reqHour <= 6 ? '6hrs' : 'fullday');
                $manualStatus = $dailyInventory->{'manual_status_' . $durationField} ?? null;
                $soldCount = $dailyInventory->{'rooms_sold_' . $durationField} ?? 0;
                if ($manualStatus === 'Sold Out') {
                    $roomtimeIds[] = $id;
                    continue;
                }
                // Determine maximum available capacity based on inventory row
                $totalRoom = $dailyInventory->total_rooms - $soldCount;
            } else {
    ...
    public function getPrice(Request $request, $slug, $id)
{
    ...
    public function getDurationAvailability(Request $request, $roomId)
    {
        $checkInDate = $request->input('check_in_date');
        $checkInDate = $checkInDate ? date('Y-m-d', strtotime($checkInDate)) : date('Y-m-d');
        ...
        $prices = HourlyRoomPrice::where('room_id', $roomId)
            ->join('booking_hours', 'hourly_room_prices.hour_id', '=', 'booking_hours.id')
            ->where('hourly_room_prices.price', '!=', null)
            ->orderBy('booking_hours.serial_number')
            ->select('hourly_room_prices.*', 'booking_hours.serial_number')
        ...
        $isBlocked = $dailyInventory && $dailyInventory->status === 'Blocked';
        ...
        foreach ($prices as $priceItem) {
            ...
            if ($isBlocked) {
                $available = false;
            } elseif ($dailyInventory) {
                if ($priceItem->hour == 3 && !empty($dailyInventory->rate_3hrs)) {
                    $price = $dailyInventory->rate_3hrs;
                } elseif ($priceItem->hour == 6 && !empty($dailyInventory->rate_6hrs)) {
                    $price = $dailyInventory->rate_6hrs;
                } elseif ($priceItem->hour == 24 && !empty($dailyInventory->rate_fullday)) {
                    $price = $dailyInventory->rate_fullday;
                }

                $durationField = $priceItem->hour <= 3 ? '3hrs' : ($priceItem->hour <= 6 ? '6hrs' : 'fullday');
                $manualStatus = $dailyInventory->{'manual_status_' . $durationField} ?? null;
                $soldCount = $dailyInventory->{'rooms_sold_' . $durationField} ?? 0;
                if ($manualStatus === 'Sold Out' || $soldCount >= $dailyInventory->total_rooms) {
                    $available = false;
                }
            }

            $result[] = [
                'hour' => (int)$priceItem->hour,
                'price' => (float)$price,
                'available' => $available,
            ];
        }

        return response()->json(['durations' => $result, 'date' => $checkInDate]);
    }
