...
class BookingController extends Controller
{
    ...
    public function timeCheck(Request $request, $paymentMethod)
    {
        ...
        // Full Day bookings always checkout at a fixed 11:00 AM the next day, regardless of check-in time
        if ($hour == 24) {
    ...
    /**
     * Generate the next sequential Booking ID (e.g. SZ01, SZ02, ...), using a row lock
     * so concurrent bookings can never receive the same number.
     */
    private function generateBookingId()
    {
        $orderNumber = null;
        \Illuminate\Support\Facades\DB::transaction(function () use (&$orderNumber) {
            $settings = \Illuminate\Support\Facades\DB::table('basic_settings')->lockForUpdate()->first();
            $prefix = $settings->booking_id_prefix ?? 'SZ';
            $nextNumber = $settings->booking_id_next_number ?? 1;
            $orderNumber = $prefix . str_pad($nextNumber, 2, '0', STR_PAD_LEFT);
            \Illuminate\Support\Facades\DB::table('basic_settings')->update(['booking_id_next_number' => $nextNumber + 1]);
        });
        return $orderNumber;
    }
    ...
    public function storeData($arrData)
    {
        ...
        $orderInfo = Booking::query()->create([
            'order_number' => $this->generateBookingId(),
        ...
        request()->session()->put('booking_id', $orderInfo->id);
    ...
    public function complete(Request $request,$type = null)
    {
        ...
        if (empty($id)) {
            Session::flash('error', 'Booking confirmation expired. Please check your bookings for the latest status.');
            return redirect()->route(Auth::guard('web')->check() ? 'user.room_bookings' : 'frontend.rooms');
        }
        ...
        if (empty($booking)) {
            Session::flash('error', 'Booking confirmation could not be found. Please check your bookings for the latest status.');
            return redirect()->route(Auth::guard('web')->check() ? 'user.room_bookings' : 'frontend.rooms');
        }
    ...
    public function cancelBooking(Request $request)
    {
        ...
        // Restore inventory: decrement the counters that were incremented when this booking was paid
        if ($booking->check_in_date && $booking->hotel_id) {
            ...
            if ($inv) {
                ...
                $newRoomsSold = max(0, $inv->rooms_sold - $roomQty);
                $newBookingsCount = max(0, $inv->bookings_count - 1);
                $newDurationSold = max(0, $currentDurationSold - $roomQty);

                \Illuminate\Support\Facades\DB::table('hotel_daily_inventories')
                    ->where('id', $inv->id)
                    ->update([
                        'rooms_sold'     => $newRoomsSold,
                        'bookings_count' => $newBookingsCount,
                        $durationField   => $newDurationSold,
                        // Only auto-revert the blanket status - manual per-duration overrides are left untouched,
                        // since those represent deliberate vendor/admin intent, not automatic booking counts
                        'status'         => ($newRoomsSold < $inv->total_rooms) ? 'Available' : $inv->status,
                        'updated_at'     => now(),
                    ]);
            }
        }
