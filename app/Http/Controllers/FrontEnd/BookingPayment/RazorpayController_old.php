...
class RazorpayController extends Controller
{
    ...
    public function notify(Request $request)
    {
        ...
        if ($success === true) {
            ...
            // Update inventory: increment rooms_sold on successful payment
            if ($bookingInfo->check_in_date && $bookingInfo->hotel_id) {
                ...
                if ($inv) {
                    \Illuminate\Support\Facades\DB::table('hotel_daily_inventories')
                        ->where('id', $inv->id)
                        ->update([
                            'rooms_sold'     => $inv->rooms_sold + ($bookingInfo->room_qty ?? 1),
                            'bookings_count' => $inv->bookings_count + 1,
                            'status'         => (($inv->rooms_sold + ($bookingInfo->room_qty ?? 1)) >= $inv->total_rooms)
                                                ? 'Sold Out' : 'Available',
                            'updated_at'     => now(),
                        ]);
                }
