  ...
  public function loginSubmit(Request $request)
  {
    ...
    if (Auth::guard('web')->attempt($credentials)) {
      ...
      if ($request->session()->has('booking_form_data')) {
          ...
          if ($priceId) {
              // Check if this booking was already completed (exists in DB)
              $alreadyBooked = \App\Models\Booking::where('user_id', Auth::guard('web')->user()->id)
              ...
              if (!$alreadyBooked) {
                  ...
                  if ($price && $price->room_id) {
                      ...
                      $rc = \App\Models\RoomContent::where('room_id', $price->room_id)
                          ...
                          ->first();
                      if ($rc) {
                          // Try to redirect to hotel detail page if we have hotel info
                          $room = \App\Models\Room::find($rc->room_id);
                          if ($room && $room->hotel_id) {
                              $hotelContent = \App\Models\HotelContent::where('hotel_id', $room->hotel_id)
                                  ...
                                  ->first();
                              if ($hotelContent) {
                                  Session::flash('success', 'Welcome back! Your booking details have been saved. Click Book Now to complete your payment.');
                                  return redirect()->route('frontend.hotel.details', [
                                      'slug' => $hotelContent->slug,
                                      'id'   => $hotelContent->hotel_id,
                                  ])->with('reopen_booking', true);
                              }
                          }
                          // Fallback to room detail page
                          Session::flash('success', 'You are logged in. Please complete your booking below.');
