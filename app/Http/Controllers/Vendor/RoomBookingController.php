...
class RoomBookingController extends Controller
{
    ...
    public function calendarView(Request $request)
    {
        ...
        $rooms = \App\Models\Room::with('room_content')
            ->where('vendor_id', $vendor_id)
            ->where('status', 1)
            ->get();
        $information['language'] = $language;
        $information['rooms'] = $rooms;
        return view('vendors.room-booking.calendar', $information);
    }
    ...
    {
        $vendor_id = Auth::guard('vendor')->user()->id;
        $query = \App\Models\Booking::with(['hotel.hotel_contents', 'hotelRoom.room_content'])
            ->where('vendor_id', $vendor_id);
        ...
        foreach ($bookings as $bk) {
            ...
            $events[] = [
                'id' => $bk->id,
                'booking_name' => $bk->booking_name,
                'room_name' => optional($rc)->name ?? 'Room',
                'hotel_name' => optional($hc)->title ?? 'Hotel',
                'title' => (optional($rc)->name ?? 'Room') . ' — ' . ($bk->booking_name ?? ''),
                'check_in_date' => $bk->check_in_date,
                'check_in_time' => $bk->check_in_time,
                'check_out_date' => $bk->check_out_date,
                'check_out_time' => $bk->check_out_time,
                'check_in_date_time' => $bk->check_in_date . 'T' . ($bk->check_in_time ?? '12:00'),
                'check_out_date_time' => $bk->check_out_date . 'T' . ($bk->check_out_time ?? '12:00'),
                'hour' => $bk->hour,
    ...
    public function report(Request $request)
    {
        ...
        if ($request->filled('payment_status')) {
            $map = ['completed' => '1', 'pending' => '0', 'rejected' => '2'];
            $paymentStatus = $map[$request->payment_status] ?? null;
        }
        $vendor_id = Auth::guard('vendor')->user()->id;
        ...
        if ($request->filled('from') && $request->filled('to')) {
            ...
            Session::put('product_orders', $records->get());

            $information['orders'] = $records->paginate(10);
        } else {
            Session::put('product_orders', null);
            $information['orders'] = [];
        }

        return view('vendors.room-booking.report', $information);
    }
    ...
    public function exportReport()
    {
        if (Session::has('product_orders')) {
            ...
            if (count($productOrders) == 0) {
                ...
                return Excel::download(new VendorReportExport($productOrders), 'report.xlsx');
            }
        } else {
            Session::flash('warning', __('No order found to export') . '!');
