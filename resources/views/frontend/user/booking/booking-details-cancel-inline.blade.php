...
@php
  $cancelNow = \Carbon\Carbon::now();
  $cancelCheckInDt = \Carbon\Carbon::parse($b->check_in_date.' '.$b->check_in_time);
  $cancelHoursLeft = $cancelNow->diffInHours($cancelCheckInDt, false);
  $cancelMinutesSinceBooking = \Carbon\Carbon::parse($b->created_at)->diffInMinutes($cancelNow);
  $canCancel = ($b->order_status ?? '') !== 'cancelled' && $cancelCheckInDt->isFuture();
  $isEligibleForRefund = $cancelHoursLeft >= 24 || $cancelMinutesSinceBooking <= 15;
@endphp

@if(($b->order_status ?? '') === 'cancelled')
<div class="bkd-card" style="border-color:#fecaca;background:#fff5f5;margin-top:14px">
...
@elseif($canCancel)
<div class="bkd-card" style="margin-top:14px">
  ...
  <div class="bkd-card-body">
    <div style="background:{{ $isEligibleForRefund ? '#f0fdf4' : '#fef2f2' }};border-left:3px solid {{ $isEligibleForRefund ? '#16a34a' : '#dc2626' }};padding:10px 12px;border-radius:6px;font-size:12px;color:#374151;line-height:1.7;margin-bottom:12px">
      <b style="color:{{ $isEligibleForRefund ? '#16a34a' : '#dc2626' }};display:block;margin-bottom:4px">
        {!! $isEligibleForRefund ? '<i class="fas fa-check-circle"></i> Eligible for Full Refund — '.$fmt($b->grand_total) : '<i class="fas fa-times-circle"></i> Not Eligible for Refund' !!}
      </b>
      {!! $cancelHoursLeft >= 24 ? 'Cancellation is more than 24 hours before check-in — full refund applies.' : ($cancelMinutesSinceBooking <= 15 ? 'You booked just '.$cancelMinutesSinceBooking.' minute(s) ago. Cancelling within 15 minutes of booking gets you a full refund.' : 'Cancellation is within 24 hours of check-in, and the 15-minute grace window has passed.') !!}
    </div>
    ...
    <form action="{{ route('frontend.room_booking.cancel_booking') }}" method="POST"
          onsubmit="return confirm('{!! $isEligibleForRefund ? 'You will receive a full refund of '.$fmt($b->grand_total).'. Continue?' : 'No refund will be issued as per policy. Are you sure you want to cancel?' !!}')">
      @csrf
