...
<div class="profile-card">
    ...
    <div class="pf-section">
        ...
        @if (count($bookings) == 0)
            <h4 class="text-center mt-4" style="color: var(--ink-light);">{{ __('NO BOOKING FOUND') . '!' }}</h4>
        @else
             <div style="display:flex;flex-direction:column;gap:.8rem;">
                
                @foreach ($bookings as $booking)
                    @php
                        // Map the old status logic to the new design's color scheme
                        if ($booking->payment_status == 0) {
                            $status_bg = '#fef3c7'; // Amber/Warning background
                            $status_color = 'var(--amber, #d97706)';
                            $payment_status = __('Pending');
                        } elseif ($booking->payment_status == 1) {
                            $status_bg = '#dcfce7'; // Green/Success background
                            $status_color = 'var(--green, #15803d)';
                            $payment_status = __('Approved');
                        } elseif ($booking->payment_status == 2) {
                            $status_bg = '#fee2e2'; // Red/Danger background
                            $status_color = 'var(--red, #dc2626)';
                            $payment_status = __('Rejected');
                        }
                    @endphp

                    <div style="border:1.5px solid var(--border, #e5e7eb);border-radius:var(--r-lg, 12px);padding:1.2rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
                        
                        <img src="{{ !empty($booking->room->image) ? asset('assets/img/rooms/' . $booking->room->image) : 'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa?auto=format&fit=crop&w=120&q=80' }}"
                            style="width:80px;height:60px;border-radius:8px;object-fit:cover;flex-shrink:0;" alt="{{ __('Room Image') }}">
                        
                        <div style="flex:1;min-width:0;">
                            <div style="font-family:'Syne',sans-serif;font-size:.92rem;font-weight:800;color:var(--ink, #111827);margin-bottom:.2rem;">
                                {{ $booking->room->title ?? __('Booking No') . ' #' . $booking->order_number }}
                            </div>
                            
                            <div style="font-size:.75rem;color:var(--ink-light, #6b7280);margin-bottom:.4rem;">
                                <i class="fas fa-map-marker-alt" style="color:var(--purple, #6366f1);font-size:.65rem;margin-right:.2rem;"></i>
                                {{ $booking->booking_address ?? __('Location not specified') }}
                            </div>
                            
                            <div style="display:flex;flex-wrap:wrap;gap:.4rem;">
                                <span style="background:#f3f4f6;border-radius:5px;padding:2px 8px;font-size:.68rem;font-weight:600;color:var(--ink-mid, #4b5563);">
                                ...
                                <span style="background:{{ $status_bg }};border-radius:5px;padding:2px 8px;font-size:.68rem;font-weight:700;color:{{ $status_color }};">
                                    {{ $payment_status }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div style="text-align:right;flex-shrink:0;">
                            <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:900;color:var(--purple, #6366f1);">
                                {{ symbolPrice($booking->grand_total ?? 0) }}
                            </div>
                            
                             
                            <a                                        style="margin-top:.4rem;border:1.5px solid var(--purple);background:var(--purple-light);color:var(--purple);border-radius:8px;padding:4px 12px;font-size:.72rem;font-weight:700;cursor:pointer;">                                            {{ __('View Details') }}
                            </a>
                        </div>
                    </div>
                @endforeach
