@php
    ...
    // Fallback: if this room has no room-specific gallery photos, use the hotel-level gallery instead
    if ($roomImages->count() > 0) {
        $galleryImages = $roomImages;
        $galleryFolder = 'assets/img/room/room-gallery/';
    } else {
        $galleryImages = \Illuminate\Support\Facades\DB::table('hotel_images')->where('hotel_id', $room->hotel_id)->get();
        $galleryFolder = 'assets/img/hotel/hotel-gallery/';
    }

    $checkInDates = request()->input('date_range') ?? request()->input('checkInDates');
    $checkInTime  = request()->input('check_time') ?? request()->input('checkInTime') ?? request()->input('checkInTimes'); 
    ...
    if (!empty($checkInDates)) {
        ...
        // 2. "Blocked" (holiday/maintenance) still blocks everything; "Sold Out" per-duration is handled below
        ...
        if (!$isBlocked) {
            ...
            if ($dailyInventory) {
                ...
                // 5. Per-duration sold-out filtering: a duration drops out if manually marked Sold Out
                //    for that specific slot, or if real bookings have filled capacity for it.
                //    Legacy blanket 'Sold Out' status still empties everything (safety net / backward compat).
                // Blanket 'Blocked' is already handled earlier via $isBlocked and empties everything.
                // Per-duration control is now authoritative for 'Sold Out' - no blanket override here.
                $prices = $prices->filter(function ($priceItem) use ($dailyInventory) {
    ...
    <div class="card-img-wrap">
        ...
        @if($galleryImages->count() > 0)
            <div class="img-thumbs">
                @foreach($galleryImages->take(3) as $img)
                    <div class="img-thumb">
                        {{-- 
                           CHANGE 2 & 3: 
                        ...
                        <img
                            ...
                            src="{{ asset($galleryFolder . $img->image) }}"
                            alt="{{ $room->title }}"
                            loading="lazy"
        ...
        <div class="card-footer">
            @if($prices->count() > 0)
                <div class="duration-tabs">
                    ...
                    <div class="dtab {{ (request()->filled('hour') ? $price->hour == request('hour') : $key == 0) ? 'active' : '' }}">
            ...
            <div class="book-actions">
                @php
                    ...
                    if (request()->has('date_range')) $linkParams['date_range'] = request()->input('date_range');
                    if (request()->has('check_time')) $linkParams['check_time'] = request()->input('check_time');
                    if (request()->has('hour')) $linkParams['hour'] = request()->input('hour');
                ...
                @else
                <span class="btn-book" style="opacity:0.5;cursor:not-allowed;pointer-events:none" title="No availability for this date">
                    Sold Out
                </span>
                @endif
