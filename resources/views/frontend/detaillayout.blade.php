  ...
  <div class="stayzio-loader-card">
    ...
    <div class="ss-field">
        ...
        <div class="ss-field-text">
            ...
            <select name="filter_city" style="border:none;background:none;font:inherit;font-weight:inherit;color:inherit;padding:0;margin:0;outline:none;width:100%;cursor:pointer">
                <option value="">All Cities</option>
                @foreach($ssNavCities as $ssCity)
                <option value="{{ $ssCity->id }}" @selected(request('filter_city') == $ssCity->id)>{{ $ssCity->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="ss-field">
        ...
        <div class="ss-field-text">
            ...
            <input type="date" name="date_range" min="{{ now()->format('Y-m-d') }}" value="{{ request('date_range', now()->format('Y-m-d')) }}"
                   style="border:none;background:none;font:inherit;font-weight:inherit;color:inherit;padding:0;margin:0;outline:none;width:100%">
        </div>
    </div>
    <div class="ss-field">
        ...
        <div class="ss-field-text">
            ...
            <input type="time" name="check_time" value="{{ request('check_time', '20:00') }}"
                   style="border:none;background:none;font:inherit;font-weight:inherit;color:inherit;padding:0;margin:0;outline:none;width:100%">
        </div>
    </div>
    <div class="ss-field">
        <div class="ss-field-text" style="width:100%">
            <small>Duration</small>
            <div class="ss-duration-pills">
                <label class="ss-pill"><input type="radio" name="hour" value="3" {{ request('hour','24')==3 ? 'checked' : '' }}><span>3H</span></label>
                <label class="ss-pill"><input type="radio" name="hour" value="6" {{ request('hour','24')==6 ? 'checked' : '' }}><span>6H</span></label>
                <label class="ss-pill"><input type="radio" name="hour" value="24" {{ request('hour','24')==24 ? 'checked' : '' }}><span>Day</span></label>
            </div>
        </div>
    </div>
    <button type="submit" class="ss-search-btn"><i class="fas fa-search"></i> Search</button>
</form>
<style>
.ss-duration-pills{display:flex;gap:4px;background:rgba(0,0,0,.04);border-radius:7px;padding:2px;width:100%}
.ss-pill{position:relative;flex:1;text-align:center;cursor:pointer;display:block}
.ss-pill input{position:absolute;opacity:0;width:100%;height:100%;top:0;left:0;cursor:pointer;margin:0}
.ss-pill span{display:block;padding:3px 6px;border-radius:5px;font-size:.72rem;font-weight:700;color:#8a8a8a;font-family:"Poppins",sans-serif;white-space:nowrap}
.ss-pill input:checked + span{background:var(--red);color:#fff}
</style>
