...
@section('metaKeywords'){{ $hotel->meta_keyword ?? '' }}@endsection
@section('metaDescription'){{ $hotel->meta_description ?? '' }}@endsection
@section('ogTitle'){{ $hotel->title ?? '' }}@endsection

@section('content')
@if(session('success'))
<div style="background:#dcfce7;border:1px solid #86efac;color:#15803d;padding:12px 20px;font-size:13px;font-weight:600;text-align:center;position:relative;z-index:10">
  <i class="fas fa-check-circle"></i> {{ session('success') }}
</div>
@endif
@if(session('info'))
<div style="background:#dbeafe;border:1px solid #93c5fd;color:#1d4ed8;padding:12px 20px;font-size:13px;font-weight:600;text-align:center;position:relative;z-index:10">
...
@php
  ...
  // ── Basic settings (for Google Maps) ──
  $bs = $basicInfo ?? \App\Models\BasicSettings\Basic::first();
...
.hd-wrap{max-width:1360px;margin:0 auto;padding:0 1rem}
.hd-wrap-body{max-width:1360px;margin:0 auto;padding:0 1rem 80px}
...
.hd-badge{display:inline-flex;align-items:center;gap:5px;background:#fff1f1;border:1px solid #fecaca;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;color:#e31e24}
.hd-badge i{font-size:10px}
...
.hd-tab-nav{background:#fff;border-top:1px solid #f0ebe4;border-bottom:1.5px solid #e8eaed;position:sticky;top:64px;z-index:400}
.hd-tab-nav-inner{max-width:1360px;margin:0 auto;padding:0 1rem;display:flex;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}
...
/* Body grid */
.hd-body{max-width:1360px;margin:0 auto;padding:18px 1rem 0;display:grid;grid-template-columns:1fr 340px;gap:20px;align-items:start}
...
.hd-bw-slot.hd-slot-soldout{opacity:0.45;cursor:not-allowed;background:#f3f4f6}
.hd-bw-slot.hd-slot-soldout .hd-bw-slot-pr{color:#dc3545}
.hd-bw-cta:disabled{opacity:0.5;cursor:not-allowed;pointer-events:none}
...
/* Gallery Lightbox */
.hd-lightbox{display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.95);flex-direction:column}
.hd-lb-top{display:flex;align-items:center;justify-content:space-between;padding:14px 20px;flex-shrink:0}
.hd-lb-counter{color:rgba(255,255,255,.7);font-size:13px;font-weight:600}
.hd-lb-close{background:rgba(255,255,255,.1);border:none;color:#fff;width:38px;height:38px;border-radius:50%;cursor:pointer;font-size:20px;display:flex;align-items:center;justify-content:center;transition:.2s}
.hd-lb-close:hover{background:rgba(255,255,255,.2)}
.hd-lb-main{flex:1;display:flex;align-items:center;justify-content:center;position:relative;min-height:0;padding:0 60px}
.hd-lb-main img{max-width:100%;max-height:100%;object-fit:contain;border-radius:8px;transition:opacity .2s}
.hd-lb-prev,.hd-lb-next{position:absolute;top:50%;transform:translateY(-50%);background:rgba(255,255,255,.1);border:none;color:#fff;width:44px;height:44px;border-radius:50%;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;transition:.2s}
.hd-lb-prev:hover,.hd-lb-next:hover{background:rgba(255,255,255,.25)}
.hd-lb-prev{left:12px}
.hd-lb-next{right:12px}
.hd-lb-thumbs{display:flex;gap:8px;padding:12px 20px;overflow-x:auto;flex-shrink:0;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.3) transparent}
.hd-lb-thumbs::-webkit-scrollbar{height:4px}
.hd-lb-thumbs::-webkit-scrollbar-thumb{background:rgba(255,255,255,.3);border-radius:2px}
.hd-lb-thumb{width:72px;height:52px;border-radius:6px;overflow:hidden;cursor:pointer;flex-shrink:0;border:2px solid transparent;transition:.2s;opacity:.6}
.hd-lb-thumb.active{border-color:#fff;opacity:1}
.hd-lb-thumb img{width:100%;height:100%;object-fit:cover}
@media(max-width:640px){
  .hd-lb-main{padding:0 44px}
  .hd-lb-prev{left:4px}
  .hd-lb-next{right:4px}
  .hd-lb-thumb{width:56px;height:40px}
}
...
@media(max-width:640px){
  ...
  .hd-body{padding:14px .75rem 0}
  .hd-infobar{padding:14px .75rem}
}
</style>
...
<div class="hd-wrap">
  ...
  <div class="hd-infobar">
    <div>
      ...
      {{-- Badges: run add_hotel_badges.sql first to enable --}}
    </div>
...
<div>
  ...
  <div class="hd-scard" id="sec-location">
    ...
    <div class="hd-scard-body">
      @if(isset($bs) && $bs->google_map_api_key_status == 1 && $hotel->latitude && $hotel->longitude)
      <div id="hotelMap" style="width:100%;height:220px;border-radius:10px;overflow:hidden;border:1px solid #e8eaed"></div>
      @else
      <div class="hd-map">
...
<div class="hd-right">
  ...
  <div class="hd-bwidget">
    ...
    <div class="hd-bw-form">
        ...
        <div class="hd-bw-slots" id="hdSlots">
          @foreach($rooms as $room)
            ...
            @foreach($prices as $price)
              ...
              @php
                $slotDisplayPrice = $price->price;
                if(!empty($todayInv)) {
                  if($price->hour == 3 && !empty($todayInv->rate_3hrs)) $slotDisplayPrice = $todayInv->rate_3hrs;
                  elseif($price->hour == 6 && !empty($todayInv->rate_6hrs)) $slotDisplayPrice = $todayInv->rate_6hrs;
                  elseif($price->hour == 24 && !empty($todayInv->rate_fullday)) $slotDisplayPrice = $todayInv->rate_fullday;
                }
              @endphp
              <div class="hd-bw-slot {{ $loop->parent->first && $loop->first ? 'active' : '' }}"
                onclick="hdSelectSlot(this,{{ $slotDisplayPrice }},'{{ $price->hour }}')"
                data-price="{{ $slotDisplayPrice }}" data-hour="{{ $price->hour }}"
                data-price-id="{{ $price->id }}">
                <span class="hd-bw-slot-hr">{{ $price->hour == 24 || strtolower($price->hour) == 'full day' ? 'Full Day' : $price->hour.' Hrs' }}</span>
                <span class="hd-bw-slot-pr">{{ $fmt($slotDisplayPrice) }}</span>
              </div>
              @endif
            @endforeach
          @endforeach
      ...
      <div>
        ...
        <input type="date" class="hd-bw-inp" id="hdCheckIn" min="{{ date('Y-m-d') }}" value="{{ request('date_range', request('checkInDates', '')) }}" onchange="hdCalc(); hdRefreshAvailability(); hdUpdateTimeMin();">
      </div>
      <div id="hdCheckOutWrap">
        <span class="hd-field-lbl">Check-out Date <small id="hdCheckoutHint" style="display:none;font-weight:400;color:var(--muted);text-transform:none;letter-spacing:normal">&middot; Fixed 11:00 AM</small></span>
        <input type="date" class="hd-bw-inp" id="hdCheckOut" min="{{ date('Y-m-d', strtotime('+1 day')) }}" onchange="hdCalc()">
      </div>
      <div>
        <span class="hd-field-lbl">Check-in Time <small id="hdCheckinHint" style="display:none;font-weight:400;color:var(--muted);text-transform:none;letter-spacing:normal">&middot; Full Day: any time, checkout 11 AM next day</small></span>
        <input type="time" class="hd-bw-inp" id="hdTime" value="{{ ($szT = request('check_time', request('checkInTime', request('checkInTimes', '')))) ? date('H:i', strtotime($szT)) : '10:00' }}">
      </div>
    ...
    <div class="hd-bw-cta-wrap" style="padding:12px 18px 16px">
      <button class="hd-bw-cta" onclick="hdOpenBookingModal()">
  ...
  <div class="hd-cancel-card">
    <h5><i class="fas fa-check-circle"></i> Cancellation Policy</h5>
    <p>
      <strong>Full refund</strong> if cancelled <strong>24+ hours</strong> before check-in.<br>
      <strong>Full refund</strong> if cancelled within <strong>15 minutes</strong> of booking (mistake booking).<br>
      No refund within 24 hours of check-in (applies to all booking types — 3 hr, 6 hr, Full Day).<br>
...
<div class="hd-mob-bar" id="hdMobBar">
  ...
  <button class="hd-mob-cta" onclick="hdOpenDrawer()">
    <i class="fas fa-calendar-check"></i> Check Price
  </button>
</div>
...
<div class="hd-drawer" id="hdDrawer" onclick="hdCloseDrawerOutside(event)">
  ...
  <div class="hd-drawer-panel" id="hdPanel">
    ...
    <div style="padding:14px 18px">
      ...
      <button class="hd-bw-cta" style="margin-bottom:10px" onclick="hdOpenBookingModal()">
        <i class="fas fa-lock"></i> Book Now
      </button>
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:10px 12px;font-size:11px;color:#166534">
        ...
        Free cancellation 24h before check-in · 15-min grace window for mistakes
      </div>
    </div>
  </div>
</div>
...
<div id="hdBookModal" style="display:none;position:fixed;inset:0;z-index:700;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.55)">
  <div id="hdBookPanel" style="background:#fff;border-radius:22px 22px 0 0;width:100%;max-width:560px;margin:0 auto;max-height:92vh;overflow-y:auto;transform:translateY(100%);transition:transform .35s cubic-bezier(.4,0,.2,1)">

    {{-- Handle --}}
    <div style="width:40px;height:4px;background:#e5e7eb;border-radius:2px;margin:14px auto 0"></div>
    ...
    <div style="padding:16px 20px 12px;border-bottom:1px solid #f3f4f6;display:flex;align-items:center;justify-content:space-between">
      <div>
        <div style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#94a3b8;margin-bottom:2px">Confirm your booking</div>
        <div style="font-size:17px;font-weight:800;color:#0c0c0e;font-family:'Poppins',sans-serif">{{ $hotel->title }}</div>
      </div>
      <button onclick="hdCloseBookModal()" style="background:#f3f4f6;border:none;width:34px;height:34px;border-radius:50%;cursor:pointer;font-size:16px;color:#64748b;display:flex;align-items:center;justify-content:center">&times;</button>
    </div>
    ...
    <div style="padding:18px 20px">
      ...
      <div style="background:#f8fafc;border:1px solid #e8eaed;border-radius:12px;padding:14px 16px;margin-bottom:16px">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div>
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Duration</div>
            <div style="font-size:14px;font-weight:800;color:#0c0c0e;font-family:'Poppins',sans-serif" id="bm-slot">—</div>
          </div>
          <div>
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Room</div>
            <div style="font-size:13px;font-weight:700;color:#0c0c0e">Standard Room</div>
          </div>
          <div>
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Check-in</div>
            <div style="font-size:13px;font-weight:700;color:#0c0c0e" id="bm-checkin">—</div>
          </div>
          <div id="bm-checkout-wrap">
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Check-out</div>
            <div style="font-size:13px;font-weight:700;color:#0c0c0e" id="bm-checkout">—</div>
          </div>
          <div>
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Rooms</div>
            <div style="font-size:13px;font-weight:700;color:#0c0c0e" id="bm-rooms">—</div>
          </div>
          <div>
            <div style="font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;margin-bottom:3px">Adults</div>
            <div style="font-size:13px;font-weight:700;color:#0c0c0e" id="bm-adults">—</div>
          </div>
        </div>
      </div>

      {{-- Price breakdown --}}
      <div style="background:#fff;border:1px solid #e8eaed;border-radius:12px;overflow:hidden;margin-bottom:16px">
        <div style="padding:12px 16px;border-bottom:1px solid #f3f4f6;display:flex;justify-content:space-between;font-size:12px;color:#64748b">
          <span id="bm-price-label">Room price</span><span id="bm-price-val">—</span>
        </div>
        <div id="bm-addon-row" style="display:none;padding:12px 16px;border-bottom:1px solid #f3f4f6;display:flex;justify-content:space-between;font-size:12px;color:#64748b">
          <span>Add-ons</span><span id="bm-addon-val">—</span>
        </div>
        <div style="padding:12px 16px;border-bottom:1px solid #f3f4f6;display:flex;justify-content:space-between;font-size:12px;color:#64748b">
          <span>GST (5%)</span><span id="bm-gst-val">—</span>
        </div>
        <div style="padding:14px 16px;display:flex;justify-content:space-between;font-size:16px;font-weight:900;color:#0c0c0e;font-family:'Poppins',sans-serif">
          <span>Total Payable</span><span id="bm-total-val" style="color:var(--red)">—</span>
        </div>
      </div>

      {{-- Cancellation note --}}
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:10px 13px;margin-bottom:18px;display:flex;align-items:flex-start;gap:8px">
        <i class="fas fa-check-circle" style="color:#059669;margin-top:1px;flex-shrink:0"></i>
        <div style="font-size:11px;color:#166534;line-height:1.6">
          Free cancellation 24h+ before check-in. 15-min grace window for mistake bookings. No refund within 24h of check-in.
        </div>
      </div>

      {{-- Hidden booking form --}}
      <form id="hdBookForm" action="{{ route('frontend.room.room_booking') }}" method="POST">
        @csrf
        <input type="hidden" name="room_id"     id="bm-room-id"     value="{{ $rooms->first()->id ?? '' }}">
        <input type="hidden" name="price"        id="bm-price-id"    value="">
        <input type="hidden" name="room_qty"     id="bm-room-qty"    value="">
        <input type="hidden" name="adult"        id="bm-adult"       value="">
        <input type="hidden" name="children"     value="0">
        <input type="hidden" name="checkInDate"  id="bm-ci-date"     value="">
        <input type="hidden" name="checkOutDate" id="bm-co-date"     value="">
        <input type="hidden" name="checkInTime"  id="bm-ci-time"     value="">
        <input type="hidden" name="gateway"      value="razorpay">

        <button type="submit" id="hdBookSubmitBtn"
          style="width:100%;background:var(--red);color:#fff;border:none;border-radius:12px;padding:15px;font-family:'Poppins',sans-serif;font-size:15px;font-weight:800;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;box-shadow:0 4px 18px rgba(227,30,36,.35);transition:all .2s">
          <i class="fas fa-lock"></i> Confirm & Pay — <span id="bm-cta-amt">—</span>
        </button>
      </form>

      <div style="text-align:center;margin-top:10px;font-size:11px;color:#94a3b8;display:flex;align-items:center;justify-content:center;gap:5px">
        <i class="fas fa-shield-alt" style="color:#059669"></i> Secured by Razorpay · 100% Safe
      </div>
    </div>
  </div>
</div>
...
<div id="hdLightbox" class="hd-lightbox">
  <div class="hd-lb-top">
    <div style="font-size:15px;font-weight:800;color:#fff">{{ $hotel->title }}</div>
    <div style="display:flex;align-items:center;gap:12px">
      <span class="hd-lb-counter"><span id="hdLbCurrent">1</span> / <span id="hdLbTotal">0</span></span>
      <button class="hd-lb-close" onclick="closeLightbox()">&#215;</button>
    </div>
  </div>
  <div class="hd-lb-main">
    <button class="hd-lb-prev" onclick="lbNav(-1)"><i class="fas fa-chevron-left"></i></button>
    <img id="hdLbImg" src="" alt="Gallery">
    <button class="hd-lb-next" onclick="lbNav(1)"><i class="fas fa-chevron-right"></i></button>
  </div>
  <div class="hd-lb-thumbs" id="hdLbThumbs"></div>
</div>
...
var _hdPrice = 0, _hdSlot = '', _hdNights = 1, _hdRooms = 2, _hdAddon = 0;
  // Price map from DB: hourly_room_prices
  var _hdPriceMap = {
    96: { hour: 3,  label: '3 Hours',  price: 1200 },
    97: { hour: 6,  label: '6 Hours',  price: 1500 },
    98: { hour: 24, label: 'Full Day', price: 2000 }
  };
  var _hdActivePriceId = 96; // default first slot
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  if(firstSlot) {
    ...
    _hdActivePriceId = firstSlot.dataset.priceId || 96;
    hdCalc();
  }
  ...
  // If user was redirected back after login with saved booking data, reopen modal
  var urlParams = new URLSearchParams(window.location.search);
  if(urlParams.get('reopen_booking') === '1') {
    setTimeout(function() {
      hdOpenBookingModal();
...
function hdSelectSlot(el, price, hour) {
  ...
  _hdActivePriceId = el.dataset.priceId || _hdActivePriceId;
  var isFullDaySelection = (hour==24||hour=='Full Day');
  var cow = document.getElementById('hdCheckOutWrap');
  if(cow) cow.style.display = isFullDaySelection ? '' : 'none';
  var ciHint = document.getElementById('hdCheckinHint');
  var coHint = document.getElementById('hdCheckoutHint');
  if(ciHint) ciHint.style.display = isFullDaySelection ? 'inline' : 'none';
  if(coHint) coHint.style.display = isFullDaySelection ? 'inline' : 'none';
  hdCalc();
}
...
function hdCalc() {
  ...
  var rm = parseInt(document.getElementById('hdRooms')?.value||1);
  _hdRooms = rm;
...
function hdUpdateTimeMin() {
  var ciInput = document.getElementById('hdCheckIn');
  var timeInput = document.getElementById('hdTime');
  if (!ciInput || !timeInput) return;
  var today = new Date();
  var todayStr = today.getFullYear() + '-' + String(today.getMonth()+1).padStart(2,'0') + '-' + String(today.getDate()).padStart(2,'0');
  if (ciInput.value === todayStr) {
    var minTime = String(today.getHours()).padStart(2,'0') + ':' + String(today.getMinutes()).padStart(2,'0');
...
document.addEventListener('DOMContentLoaded', function() { hdUpdateTimeMin(); });

function hdRefreshAvailability() {
  var roomId = document.getElementById('bm-room-id')?.value;
  var checkInDate = document.getElementById('hdCheckIn')?.value;
  if (!roomId) return;

  var url = "{{ url('/room-duration-availability') }}/" + roomId;
  if (checkInDate) url += "?check_in_date=" + encodeURIComponent(checkInDate);
  ...
  fetch(url)
    ...
    .then(function(data) {
      ...
      var activeSlotStillAvailable = false;
      var firstAvailableSlot = null;
      var sym = '{{ $sym }}';

      data.durations.forEach(function(d) {
        var slot = document.querySelector('.hd-bw-slot[data-hour="' + d.hour + '"]');
        if (!slot) return;

        var priceLabel = slot.querySelector('.hd-bw-slot-pr');

        if (d.available) {
          anyAvailable = true;
          if (!firstAvailableSlot) firstAvailableSlot = slot;
          slot.classList.remove('hd-slot-soldout');
          slot.dataset.price = d.price;
          if (priceLabel) priceLabel.textContent = sym + Math.round(d.price).toLocaleString();
          if (slot.classList.contains('active')) activeSlotStillAvailable = true;
        } else {
          slot.classList.add('hd-slot-soldout');
          if (priceLabel) priceLabel.textContent = 'Sold Out';
        }
      });
      ...
      if (!activeSlotStillAvailable) {
        ...
        if (firstAvailableSlot) {
          ...
          // Directly set the state hdCalc() reads, rather than depending on hdSelectSlot's internals
          try {
            window._hdPrice = parseFloat(firstAvailableSlot.dataset.price);
            window._hdSlot = firstAvailableSlot.dataset.hour;
            var cow = document.getElementById('hdCheckOutWrap');
            if (cow) cow.style.display = (window._hdSlot == 24 || window._hdSlot == 'Full Day') ? '' : 'none';
            if (typeof hdCalc === 'function') hdCalc();
          } catch (e) {}
        }
      }

      window._hdDurationAvailability = data.durations;

      document.querySelectorAll('.hd-bw-cta').forEach(function(btn) {
...
function hdMCalc() {
  ...
  var rm = parseInt(document.getElementById('hdMRooms')?.value||1);
...
// ── BOOKING MODAL ──────────────────────────────
function hdOpenBookingModal() {
  // Hard safety check: block if the currently selected duration is sold out,
  // regardless of any UI display state - this is the final gatekeeper before payment.
  if (window._hdDurationAvailability) {
    var currentHourNum = parseInt(_hdSlot) || 24;
    var matchingDuration = window._hdDurationAvailability.find(function(d) { return d.hour == currentHourNum; });
    if (matchingDuration && matchingDuration.available === false) {
      alert('Sorry, this duration is no longer available for the selected date. Please choose a different duration or date.');
      return;
    }
  }

  // Populate modal from current widget state
  var sym = '{{ $sym }}';
  var isFullDay = (_hdSlot == 24 || String(_hdSlot).toLowerCase().includes('full'));

  // Slot label
  document.getElementById('bm-slot').textContent = isFullDay ? 'Full Day' : _hdSlot + ' Hours';

  // Dates
  var ciDate = document.getElementById('hdCheckIn')?.value || document.getElementById('hdMCheckIn')?.value || '';
  var coDate = document.getElementById('hdCheckOut')?.value || document.getElementById('hdMCheckOut')?.value || '';
  var ciTime = document.getElementById('hdTime')?.value || '10:00';
  var rooms  = parseInt(document.getElementById('hdRooms')?.value || document.getElementById('hdMRooms')?.value || 1);
  var adults = parseInt(document.getElementById('hdAdults')?.value || 2);
  ...
  document.getElementById('bm-checkin').textContent  = fmtDate(ciDate) + (ciTime ? ' · ' + fmtTime(ciTime) : '');
  document.getElementById('bm-rooms').textContent    = rooms + ' Room' + (rooms > 1 ? 's' : '');
  document.getElementById('bm-adults').textContent   = adults + ' Adult' + (adults > 1 ? 's' : '');

  var coWrap = document.getElementById('bm-checkout-wrap');
  if (isFullDay && coDate) {
    document.getElementById('bm-checkout').textContent = fmtDate(coDate) + ' · 11:00 AM';
  ...
  // Price breakdown
  var sub   = _hdPrice * _hdNights * rooms;
  var gst   = Math.round(sub * 0.05);
  var grand = sub + _hdAddon + gst;

  var priceLabel = isFullDay
    ? 'Room × ' + _hdNights + ' night' + (_hdNights > 1 ? 's' : '') + ' × ' + rooms
    : 'Room × ' + rooms + ' room' + (rooms > 1 ? 's' : '');

  document.getElementById('bm-price-label').textContent = priceLabel;
  document.getElementById('bm-price-val').textContent   = sym + sub.toLocaleString();
  document.getElementById('bm-gst-val').textContent     = sym + gst.toLocaleString();
  document.getElementById('bm-total-val').textContent   = sym + grand.toLocaleString();
  document.getElementById('bm-cta-amt').textContent     = sym + grand.toLocaleString();

  var ar = document.getElementById('bm-addon-row');
  if(_hdAddon > 0 && ar) {
    ar.style.display = 'flex';
    document.getElementById('bm-addon-val').textContent = sym + _hdAddon.toLocaleString();
  } else if(ar) { ar.style.display = 'none'; }

  // Populate hidden form fields
  document.getElementById('bm-price-id').value  = _hdActivePriceId;
  document.getElementById('bm-room-qty').value  = rooms;
  document.getElementById('bm-adult').value     = adults;
  document.getElementById('bm-ci-date').value   = ciDate;
  document.getElementById('bm-co-date').value   = coDate;
  document.getElementById('bm-ci-time').value   = ciTime;

  // Show modal
  var modal = document.getElementById('hdBookModal');
  modal.style.display = 'flex';
  setTimeout(function() {
    document.getElementById('hdBookPanel').style.transform = 'translateY(0)';
  }, 10);
  document.body.style.overflow = 'hidden';
}

function hdCloseBookModal() {
  document.getElementById('hdBookPanel').style.transform = 'translateY(100%)';
  setTimeout(function() {
    document.getElementById('hdBookModal').style.display = 'none';
    document.body.style.overflow = '';
  }, 350);
}

// Close modal on overlay click
document.getElementById('hdBookModal').addEventListener('click', function(e) {
  if(e.target === this) hdCloseBookModal();
});

// Prevent double submit
document.getElementById('hdBookForm').addEventListener('submit', function() {
  var btn = document.getElementById('hdBookSubmitBtn');
  btn.disabled = true;
...
_lbImages.push('{{ asset("assets/img/hotel/hotel-gallery/".$img->image) }}');
@endforeach

// Fallback: if no gallery images, use hotel logo
@if($galleryImages->count() === 0)
_lbImages.push('{{ asset("assets/img/hotel/logo/".$hotel->logo) }}');
@endif
...
function openGallery(idx) {
  ...
  _lbIdx = idx || 0;
  if (_lbIdx >= _lbImages.length) _lbIdx = 0;

  // Build thumbs
  var thumbsEl = document.getElementById('hdLbThumbs');
  thumbsEl.innerHTML = '';
  _lbImages.forEach(function(src, i) {
    var t = document.createElement('div');
    t.className = 'hd-lb-thumb' + (i === _lbIdx ? ' active' : '');
    t.innerHTML = '<img src="' + src + '" alt="' + i + '">';
    t.onclick = function() { lbGoto(i); };
    thumbsEl.appendChild(t);
  });

  document.getElementById('hdLbTotal').textContent = _lbImages.length;
  lbGoto(_lbIdx);

  var lb = document.getElementById('hdLightbox');
  lb.style.display = 'flex';
  document.body.style.overflow = 'hidden';

  // Scroll thumb into view
  setTimeout(function() {
    var active = thumbsEl.querySelector('.hd-lb-thumb.active');
    if (active) active.scrollIntoView({inline:'center', behavior:'smooth'});
  }, 100);
}

function lbGoto(idx) {
  _lbIdx = (idx + _lbImages.length) % _lbImages.length;
  var img = document.getElementById('hdLbImg');
  img.style.opacity = '0';
  setTimeout(function() {
    img.src = _lbImages[_lbIdx];
    img.style.opacity = '1';
  }, 150);
  document.getElementById('hdLbCurrent').textContent = _lbIdx + 1;
  // Update thumb active
  document.querySelectorAll('.hd-lb-thumb').forEach(function(t, i) {
    t.classList.toggle('active', i === _lbIdx);
  });
  // Scroll thumb
  var thumbsEl = document.getElementById('hdLbThumbs');
  var active = thumbsEl.querySelectorAll('.hd-lb-thumb')[_lbIdx];
  if (active) active.scrollIntoView({inline:'center', behavior:'smooth'});
}
...
function closeLightbox() {
  document.getElementById('hdLightbox').style.display = 'none';
  document.body.style.overflow = '';
}

// Close on overlay click
document.getElementById('hdLightbox').addEventListener('click', function(e) {
  if (e.target === this) closeLightbox();
});

// Keyboard navigation
document.addEventListener('keydown', function(e) {
  if (document.getElementById('hdLightbox').style.display === 'flex') {
...
function initHotelMap() {
  var lat = parseFloat('{{ $hotel->latitude ?? 0 }}');
  var lng = parseFloat('{{ $hotel->longitude ?? 0 }}');
  if(!lat || !lng) return;
  ...
  var marker = new google.maps.Marker({
    ...
    map: map,
    title: '{{ addslashes($hotel->title) }}',
    icon: {
      url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="#e31e24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>'),
      scaledSize: new google.maps.Size(36, 36),
      anchor: new google.maps.Point(18, 36)
    }
  });
  new google.maps.InfoWindow({
    content: '<div style="padding:8px 4px;font-family:Poppins,sans-serif"><strong style="font-size:13px">{{ addslashes($hotel->title) }}</strong><br><span style="font-size:11px;color:#64748b">{{ addslashes($hotel->address) }}</span></div>'
  }).open(map, marker);
}
</script>
<script src="https://maps.googleapis.com/maps/api/js?key={{ $bs->google_map_api_key }}&callback=initHotelMap" async defer></script>
@endif
@endsection