...
<div class="card">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            @if($vn)<a href="{{ route('admin.vendor_management.vendor_details',['id'=>$vn->id,'language'=>$defaultLang->code]) }}" style="color:var(--sz-blue);font-weight:500;text-decoration:none">{{ $vn->username }}</a>
          ...
          <td class="fw6" style="color:var(--sz-text)">{{ $t->currency_symbol }}{{ number_format($t->grand_total,0) }}</td>
  ...
  <div class="pagination">{{ $transcations->appends(request()->query())->links() }}<span class="pg-info">{{ $transcations->firstItem() }}–{{ $transcations->lastItem() }} of {{ $transcations->total() }}</span></div>
</div>
@endsection


{{-- ============================================================
     resources/views/admin/payment_log.blade.php (Subscriptions)
     ============================================================ --}}
@extends('admin.layout')
@section('section','Finance')
...
<div class="card">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            @if($vn)<a href="{{ route('admin.vendor_management.vendor_details',['id'=>$vn->id,'language'=>$defaultLang->code]) }}" style="color:var(--sz-blue);font-weight:500;text-decoration:none">{{ $vn->username }}</a>@else—@endif
          </td>
  ...
  <div class="pagination">{{ $memberships->appends(request()->query())->links() }}<span class="pg-info">{{ $memberships->firstItem() }}–{{ $memberships->lastItem() }} of {{ $memberships->total() }}</span></div>
</div>
@endsection
