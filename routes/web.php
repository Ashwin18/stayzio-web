...
use App\Http\Controllers\Vendor\VendorHourlyInventoryController;

// Replace 'auth' with your actual vendor authentication guard or middleware if different
Route::prefix('vendor')->name('vendor.')->middleware(['auth'])->group(function () {
    Route::prefix('hotel/inventory')->name('hotel.inventory.')->group(function () {
        
        // Route Path: /vendor/hotel-management/inventory/hourly
        Route::get('hourly', [VendorHourlyInventoryController::class, 'index'])->name('hourly.index');
        
        // Route Path: /vendor/hotel-management/inventory/update-inline
        Route::post('update-inline', [VendorHourlyInventoryController::class, 'updateInline'])->name('update_inline');
        
    });
});

Route::get('/api/nearby-rooms', [App\Http\Controllers\FrontEnd\HomeController::class, 'getNearbyRooms'])->name('frontend.nearby_rooms');
Route::group(['prefix' => 'vendor/hotel', 'as' => 'vendor.hotel.'], function () {
    Route::get('/hourly', [VendorHourlyInventoryController::class, 'index'])->name('inventory.index');
    Route::post('/update-inline', [VendorHourlyInventoryController::class, 'updateInline'])->name('inventory.update_inline');
});




Route::get('/areas/search', 'FrontEnd\AreaSearchController@search')->name('areas.search');

Route::get('/room-duration-availability/{roomId}', 'FrontEnd\RoomController@getDurationAvailability')->name('frontend.room.duration_availability');

Route::get('/search/suggest', 'FrontEnd\AreaSearchController@suggest')->name('search.suggest');

Route::get('/search/suggest', 'FrontEnd\AreaSearchController@suggest')->name('search.suggest');

Route::group(['prefix' => 'admin/hotel-management', 'as' => 'admin.hotel_management.'], function () {
    Route::get('/hourly-inventory', [HourlyInventoryController::class, 'index'])->name('hourly_inventory.index');
    Route::post('/hourly-inventory/update', [HourlyInventoryController::class, 'updateInline'])->name('inventory.update_inline');
});
...
Route::middleware('change.lang')->group(function () {
  ...
  Route::prefix('room')->group(function () {
    ...
    Route::prefix('/room-booking')->group(function () {
      ...
      Route::post('/cancel-booking', 'FrontEnd\BookingPayment\BookingController@cancelBooking')->name('frontend.room_booking.cancel_booking');
    });
  });
...
Route::get('/user/otp-login', 'FrontEnd\OtpAuthController@show')->name('user.otp.show');
Route::post('/user/otp-login/send', 'FrontEnd\OtpAuthController@sendOtp')->name('user.otp.send');
Route::post('/user/otp-login/verify', 'FrontEnd\OtpAuthController@verifyOtp')->name('user.otp.verify');
Route::post('/user/otp-login/complete-signup', 'FrontEnd\OtpAuthController@completeSignup')->name('user.otp.complete_signup');
